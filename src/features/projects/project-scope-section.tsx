...
export function ProjectScopeSection({
  ...
  async function handleAdd() {
    if (!value.trim()) { toast.error("Description is required"); return; }
    setSubmitting(true);
    const res = await fetch("/api/scope-items", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ description: value.trim(), projectId }),
    });
    setSubmitting(false);
    if (!res.ok) { toast.error("Failed to add scope item"); return; }
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    const res = await fetch(`/api/scope-items/${id}`, { method: "DELETE" });
    setDeletingId(null);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <CardTitle className="flex items-center gap-2">
          <ClipboardList className="h-4 w-4 text-muted-foreground" /> Our Scope
        </CardTitle>
        {canManage && !adding && (
        ...
        {adding && (
          <div className="space-y-2 rounded-lg border border-border p-3">
            ...
            <div className="flex justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={() => { setAdding(false); setValue(""); }}>Cancel</Button>
          ...
          <ul className="divide-y divide-border">
            {items.map((item) => (
              <li key={item.id} className="flex items-start justify-between gap-3 py-2.5 text-sm">
                ...
                {canManage && (
                  <Button
                    variant="ghost" size="icon" onClick={() => handleDelete(item.id)} disabled={deletingId === item.id}
                  >
