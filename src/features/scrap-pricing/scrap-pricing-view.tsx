...
import { FormulasDialogTrigger, RowFormulaButton, QUICK_TOOLTIPS, type FormulaScope } from "./scrap-pricing-formulas";
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium text-muted-foreground">Totals across the current nesting result</p>
            <FormulasDialogTrigger scope={{ type: "totals", totals: result.totals, inputs } satisfies FormulaScope} />
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
            <Stat label="Used Area" value={`${fmt(result.totals.totalUsedAreaSqm, 3)} m²`} tooltip={QUICK_TOOLTIPS.usedArea} />
            <Stat label="Used Weight" value={`${fmt(result.totals.totalUsedWeightKg, 1)} kg`} tooltip={QUICK_TOOLTIPS.usedWeight} />
            <Stat label="Buy Weight" value={`${fmt(result.totals.totalBuyWeightKg, 1)} kg`} tooltip={QUICK_TOOLTIPS.buyWeight} />
            <Stat label="Used Later" value={`${fmt(result.totals.totalWeightUsedLaterKg, 1)} kg`} tooltip={QUICK_TOOLTIPS.usedLater} />
            <Stat label="Actual Scrap" value={`${fmt(result.totals.totalActualScrapWeightKg, 1)} kg`} tooltip={QUICK_TOOLTIPS.actualScrap} />
            <Stat label="Buy Cost" value={`${fmt(result.totals.totalBuyCostLE, 0)} LE`} tooltip={QUICK_TOOLTIPS.buyCost} />
            <Stat label="Used Later Value" value={`${fmt(result.totals.totalUsedLaterValueLE, 0)} LE`} tooltip={QUICK_TOOLTIPS.usedLaterValue} />
            <Stat label="Scrap Value" value={`${fmt(result.totals.totalScrapValueLE, 0)} LE`} tooltip={QUICK_TOOLTIPS.scrapValue} />
            <Stat label="Scrap % (Bought)" value={pct(result.totals.actualScrapPctFromBought)} tooltip={QUICK_TOOLTIPS.scrapPctBought} />
            <Stat label="Scrap % (Used)" value={pct(result.totals.actualScrapPctFromUsed)} tooltip={QUICK_TOOLTIPS.scrapPctUsed} />
          </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
                  ...
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.usedArea}>Used Area (m²)</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.usedWeight}>Used Wt (kg)</th>
                  <th className="px-3 py-2 text-right font-medium">Buy Qty</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.buyWeight}>Buy Wt (kg)</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.primaryScrapPct}>Primary Scrap %</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.usedLater}>Used Later Wt</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.usedLaterValue}>Used Later Value</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.actualScrap}>Actual Scrap Wt</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.scrapValue}>Scrap Value</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.buyCost}>Buy Cost</th>
                  <th className="px-3 py-2 text-right font-medium" title={QUICK_TOOLTIPS.actualScrapPct}>Actual Scrap %</th>
...
function Stat({ label, value, tooltip }: { label: string; value: string; tooltip?: string }) {
  return (
    <div className="rounded-lg border border-border p-2.5" title={tooltip}>
